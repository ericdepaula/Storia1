...
async function refinarTermosDeBusca(promptData) {
  ...
  const promptDeRefinamento = `
    ...
    Sua tarefa é traduzir esses termos, que podem ser genéricos, para categorias de mercado profissionais e específicas que um especialista em marketing usaria para uma pesquisa de tendências.
...
async function orquestrarGeracaoDeConteudo(promptData, duracaoTotalDias) {
  ...
  const termosRefindos = await refinarTermosDeBusca(promptData);
  // const anoDePesquisa = new Date().getFullYear();
  console.log("🔍 Termos refinados para pesquisa:", termosRefindos);

  // --- ETAPA DE PESQUISA ACONTECE AQUI ---
  console.log("Iniciando pesquisa de tendências...");
  const termoDeBusca = `estratégias de marketing de conteúdo para ${termosRefindos.tipoNegocio} com foco em ${termosRefindos.objetivoPrincipal}`;
